...
- `RoundTracker.js` — the per-bottle state machine (mirrors `../GameTracker.js`'s role) that turns a *sequence* of frames into `round_start`/`round_ready`/`round_end`/`piece_entered` events. One instance per bottle (versus mode uses two). Unlike GameTracker it doesn't need a score/lines heuristic to find round boundaries — `ResultOCR`'s state already gives that directly — and it tracks virus count as two-phase (up during population toward `4*(level+1)`, capped at 84 for level 21+, then down) rather than monotonic. `piece_entered` fires when a player-controlled piece spawns, detected by position (not by diffing the next-pill preview, which "rushes" — long runs of the identical pill — would defeat) and scoped specifically to the fixed pair of middle columns a real spawn always uses (confirmed report), *not* the whole top row — an earlier version watched the whole row and misread the player shifting/rotating a piece still sitting in row 0 as repeated new spawns, and scoping to just the spawn pair also means it won't be fooled by versus-mode "garbage" (always a loose single half, never that paired shape) once that's detected, which it isn't yet. A frame with `hasBottle: false` (see `ScreenOCR.js`) is a complete no-op — pause mid-round doesn't look like a round boundary, and a title/menu screen in front of the capture can't bootstrap a bogus `round_start`. Pure state-machine logic, no OCR/pixels involved — tested against hand-built frame sequences, not image fixtures.
...
- `RoundTracker`'s `piece_entered` can still double-count a piece that shifts away from the spawn-column pair and back again before locking (rarer than the whole-row false-positive bug it replaced; not closed, see the comment at the top of `RoundTracker.js`).
