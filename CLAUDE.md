...
**Known gaps** (useful to know before assuming something is handled): a cell mid-match-clear is recognized (`type: 'clearing'`, a hollow-ring shape identical for both viruses and pills — see `templates.js` `CLEARING_TEMPLATE`), but only one visual frame of it has been captured, so a second blink state (if the game has one) isn't accounted for; round-clear and game-over states aren't detected at all; only single-player and versus layouts exist (no other Dr. Mario modes have been captured).
