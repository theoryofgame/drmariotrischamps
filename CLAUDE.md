...
- `RoundTracker.js` — the per-bottle state machine (mirrors `../GameTracker.js`'s role) that turns a *sequence* of frames into `round_start`/`round_ready`/`round_end`/`piece_entered` events. One instance per bottle (versus mode uses two). Unlike GameTracker it doesn't need a score/lines heuristic to find round boundaries — `ResultOCR`'s state already gives that directly — and it tracks virus count as two-phase (up during population toward `4*(level+1)`, then down) rather than monotonic. `piece_entered` is detected by watching the whole top row for a cell going from empty to holding a pill half (position-based, not by diffing the next-pill preview), specifically because Dr. Mario "rushes" (long runs of the identical pill) would defeat a diff-based check. Pure state-machine logic, no OCR/pixels involved — tested against hand-built frame sequences, not image fixtures.
